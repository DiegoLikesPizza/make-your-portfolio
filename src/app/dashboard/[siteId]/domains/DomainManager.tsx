  ...
  return (
    <>
      <section className="mt-8">
        <h2 className="text-xs font-medium uppercase tracking-[0.08em] text-neutral-500">Your own domain</h2>
        ...
        <form action={action} className="mt-3 flex gap-2">
          <input
            ...
            autoCorrect="off"
            spellCheck={false}
            className="min-w-0 flex-1 rounded-lg border border-neutral-300 bg-white px-4 py-2.5 text-neutral-900 outline-none focus:border-neutral-900 dark:border-neutral-700 dark:bg-neutral-950 dark:text-white dark:focus:border-white"
          />
          <button
            type="submit"
            disabled={pending}
      ...
      {domains.length === 0 && (
        <p className="mt-6 text-sm text-neutral-500">
          No custom domains yet. Cloudflare users: set the record to <strong>DNS only</strong> (grey cloud),
          otherwise Cloudflare terminates TLS and the certificate can&apos;t be issued.
        </p>
      )}
    </>
  );
